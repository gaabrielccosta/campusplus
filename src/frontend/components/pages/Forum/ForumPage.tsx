...
const ForumPage: React.FC<IForumPageProps> = ({ username }) => {
  ...
  const handleCreateTopic = async (e: FormEvent) => {
    e.preventDefault();
    if (!newTitle.trim() || !newContent.trim() || !username) return;
  ...
  const handleAddReply = async (e: FormEvent) => {
    e.preventDefault();
    if (!selectedTopic || !replyContent.trim() || !username) return;
    ...
    try {
      const res = await api.post<Post>(`/topics/${selectedTopic.id}/posts`, dto);
      const newPost: Post = { ...res.data, createdAt: new Date(res.data.createdAt) };

      setTopics(prev =>
        prev.map(t => (t.id === selectedTopic.id ? { ...t, posts: [...t.posts, newPost] } : t))
      );
      setSelectedTopic(prev => (prev ? { ...prev, posts: [...prev.posts, newPost] } : prev));
      setReplyContent('');
    } catch (err) {
              ...
              <ul style={{ listStyle: 'none', padding: 0 }}>
                {topics.map(topic => (
                  <li key={topic.id} style={{ marginBottom: '1rem', border: '1px solid #ccc', padding: '0.5rem' }}>
                    <a href="#" onClick={e => { e.preventDefault(); setSelectedTopic(topic); }} style={{ fontSize: '1.1rem', fontWeight: 'bold', textDecoration: 'none' }}>
                      {topic.title}
                    </a>
